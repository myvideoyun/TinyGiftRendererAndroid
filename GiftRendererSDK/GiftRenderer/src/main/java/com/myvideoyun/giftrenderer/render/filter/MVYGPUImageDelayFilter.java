...
public class MVYGPUImageDelayFilter extends MVYGPUImageFilter {

    private MVYGPUImageFramebuffer[] cacheFramebuffer = new MVYGPUImageFramebuffer[2];
    private int cachePosition = 0;
    private long cacheExpireTime = 0;

    public MVYGPUImageDelayFilter(MVYGPUImageEGLContext context) {
        super(context);
    }
    ...
    protected void renderToTexture(final Buffer vertices, final Buffer textureCoordinates) {
        context.syncRunOnRenderThread(new Runnable() {
            ...
            public void run() {
                ...
                // 使用缓存帧
                MVYGPUImageFramebuffer framebuffer = cacheFramebuffer[cachePosition];
