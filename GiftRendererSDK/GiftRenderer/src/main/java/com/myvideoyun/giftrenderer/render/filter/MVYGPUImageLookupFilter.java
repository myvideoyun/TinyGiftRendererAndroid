...
public class MVYGPUImageLookupFilter extends MVYGPUImageFilter {
    ...
    public MVYGPUImageLookupFilter(MVYGPUImageEGLContext context, final Bitmap lookup) {
        super(context);
        this.lookup = lookup;
        context.syncRunOnRenderThread(new Runnable() {
            @Override
            public void run() {
                filterProgram = new MVYGLProgram(kAYGPUImageVertexShaderString, kAYGPUImageLookupFragmentShaderString);
                filterProgram.link();
